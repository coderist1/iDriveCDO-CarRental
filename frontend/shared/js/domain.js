...
(function (global) {
  ...
  function payBooking(bookingId, payload, csrf) {
    ...
    if (method === "cashless") {
      var receiptPhoto = typeof payload.receiptPhoto === "string" ? payload.receiptPhoto.trim() : "";
      if (!receiptPhoto) throw new Error("Upload a photo of your payment receipt.");
      if (!/^data:image\/(jpeg|jpg|png|webp);base64,/i.test(receiptPhoto)) {
        throw new Error("Payment receipt must be a JPG, PNG, or WebP image.");
      }
      if (receiptPhoto.length > 750000) {
        throw new Error("Payment receipt is too large. Choose a clearer, smaller photo.");
      }
      payment = {
        method: "cashless",
        brand: "QR",
        ...
        authCode: "QR" + NS.security.randomHex(3).toUpperCase(),
        paidAt: new Date().toISOString()
      };
      audit("payment", me.id, booking.ref + " cashless QR receipt");
    } else {
    ...
    var payNote =
      ...
      booking.ref +
      " via " +
      (method === "cashless" ? "QR payment" : "cash") +
      ".";
