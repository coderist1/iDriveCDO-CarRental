(function (global) {
  ...
  function readReceiptPhoto(file, done) {
    readImageFile(file, done, { maxSide: 960, maxBytes: 700000, label: "Payment receipt" });
  }
  ...
  function downloadReceipt(booking) {
    ...
    if (payment) {
      if (payment.method === "cashless") {
        paymentLine =
          payment.brand === "QR" || payment.last4 === "QR"
            ? "Cashless · QR · " + payment.authCode
            : "Cashless · " + payment.brand + " ·••" + payment.last4 + " · " + payment.authCode;
  ...
  NS.ui = {
    ...
    readReceiptPhoto: readReceiptPhoto,
